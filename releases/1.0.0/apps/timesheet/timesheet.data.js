(function (global) {
    ...
    function loadResources(state) {
        ...
        return global.AxialQB.quickbase.doQuery({
            dbid: alias(c.tableAliases.resources), appToken: c.appToken, query: query,
            clist: [r.id, r.name, r.active].join("."), slist: r.name,
            options: "sortorder-A", eventName: "timesheet.resources.failed"
        }).then(function (result) {
            var rows = [];
            global.jQuery(result.data).find("record").each(function () {
                rows.push({ id: f(this, r.id), name: f(this, r.name), active: f(this, r.active) });
            });
    ...
    function loadTasks(state) {
        ...
        return global.AxialQB.quickbase.doQuery({
            dbid: alias(c.tableAliases.tasks), appToken: c.appToken, query: query,
            clist: clist, slist: (c.taskSortFields || []).join("."), options: "sortorder-D", eventName: "timesheet.tasks.failed"
        }).then(function (result) {
            var rows = [];
            global.jQuery(result.data).find("record").each(function () {
                rows.push({
                    id:f(this,t.id), taskName:f(this,t.taskName), projectCode:f(this,t.projectCode),
                    projectDescription:f(this,t.projectDescription), budgetedHours:num(f(this,t.budgetedHours)),
                    assignedHours:num(f(this,t.assignedHours)), remainingHours:num(f(this,t.remainingHours)),
                    loadedRemainingHours:num(f(this,t.remainingHours)), projectType:f(this,t.projectType) || "Other",
                    startDate:qbDate(f(this,t.startDate)), endDate:qbDate(f(this,t.endDate)),
                    lastEntryDate:qbDate(f(this,t.lastEntryDate)), status:f(this,t.status),
                    allowOverbudget:f(this,t.allowOverbudget)==="1", allowOvertime:f(this,t.allowOvertime)==="1"
                });
            });
            state.tasks = rows;
            return rows;
    ...
    function loadEntries(state) {
        ...
        state.entries = {};
        state.tasks.forEach(function (task) { state.entries[task.id] = []; });
        if (!resource || !state.periods.length) { return Promise.resolve(state.entries); }
        ...
        return global.AxialQB.quickbase.doQuery({
            dbid: alias(c.tableAliases.timeEntries), appToken:c.appToken, query:query,
            clist:clist, slist:e.task, eventName:"timesheet.entries.failed"
        }).then(function (result) {
            global.jQuery(result.data).find("record").each(function () {
                var taskId = f(this,e.task), date = qbDate(f(this,e.date));
                if (date) { date.setHours(0,0,0,0); }
                if (!state.entries[taskId]) { state.entries[taskId] = []; }
                state.entries[taskId].push({
                    id:f(this,e.id), taskId:taskId, standardHours:f(this,e.standardHours),
                    standardNote:f(this,e.standardNote), overtimeHours:f(this,e.overtimeHours),
                    internalComment:f(this,e.internalComment), date:date,
                    approved:f(this,e.approved)==="1", payrollApproved:f(this,e.payrollApproved)==="1", dirty:false
                });
            });
    ...
    function save(state) {
        ...
        return global.AxialQB.quickbase.importCsv({
            dbid:alias(c.tableAliases.timeEntries),appToken:c.appToken,csv:csv,
            clist:[e.id,e.task,e.standardHours,e.standardNote,e.overtimeHours,e.internalComment,e.date].join("."),
            eventName:"timesheet.save.failed"
